...
exports.abtester = new abTester_1.ABTester(contextForMaster, { ...options, retries: 3 });
...
exports.promptProductionWorkspace = async (promptMessage) => {
    const productionWorkspaces = await clients_1.workspaces.list(account).then(ramda_1.compose(ramda_1.map(({ name }) => name), ramda_1.filter(({ name, production }) => production === true && name !== 'master')));
